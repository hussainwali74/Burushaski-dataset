...
awk 'NR==FNR{words[$1]; next;} !($1 in words)' \
  $locdict/urdudict-plain.txt $locdata/vocab-full.txt |\
  egrep -v '<.?s>' > $locdict/vocab-oov.txt

awk 'NR==FNR{words[$1]; next;} ($1 in words)' \
  $locdata/vocab-full.txt $locdict/urdudict-plain.txt |\
...
g2p.py --model=conf/g2p_model --encoding UTF-8 --apply $locdict/vocab-oov.txt > $locdict/lexicon-oov.txt
mv $locdict/lexicon-oov.txt $locdict/lexicon-oov-temp.txt
cat $locdict/lexicon-oov-temp.txt | awk 'NF>1{print}' > $locdict/lexicon-oov.txt

cat $locdict/lexicon-oov.txt $locdict/lexicon-iv.txt |\
  sort > $locdict/lexicon.txt

sed -i 's/_//g' $locdict/lexicon.txt
