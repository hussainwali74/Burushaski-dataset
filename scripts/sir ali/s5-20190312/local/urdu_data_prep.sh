...
echo "=== Starting initial Urdu data preparation ..."
...
cat $loctmp/speakers_all.txt | sed -n 1,${nspk_test}p | sort -u >$loctmp/speakers_test.txt

echo "test speakers" $nspk_test
cat $loctmp/speakers_test.txt
...
echo "*** Initial Urdu data preparation finished!"
