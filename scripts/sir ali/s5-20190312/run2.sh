...
local/urdu_select.sh --dialect $dialects \
...
local/urdu_data_prep.sh --nspk_test ${nspk_test} ${selected} || exit 1



# Prepare ARPA LM and vocabulary using SRILM
local/urdu_prepare_lm.sh --order ${lm_order} || exit 1



# Prepare the lexicon and various phone lists
# Pronunciations for OOV words are obtained using a pre-trained Sequitur model
local/urdu_prepare_dict.sh || exit 1
...
local/urdu_format_data.sh || exit 1
...
utils/subset_data_dir.sh data/train 2000 data/train.1k  || exit 1;
...
steps/train_deltas.sh --cmd "$train_cmd" \
  3000 11000 data/train data/lang exp/mono_ali exp/tri1 || exit 1;
...
steps/train_deltas.sh --cmd "$train_cmd" 3000 16000 \
...
steps/train_lda_mllt.sh --cmd "$train_cmd" 3000 22000 \
